 ...
 */
export const activeQuizStates = new Map<string, qi.QuizState>();
...
const globalHandlers: StateActions = {
  pong: (io, socket, data, quizState) =>
    h.handlePong(io, socket, data, quizState),
  'state:next': (io, socket, data, quizState) =>
    h.handleNextState(io, socket, data, quizState),
  'player:join': (io, socket, data, quizState) =>
    h.handlePlayerJoin(io, socket, data, quizState),
};
  ...
  io.on('connection', (socket) => {
    socket.onAny((eventName, rawBuffer: any) => {
      ...
      if (!(eventName in QuizSchemas)) {
        ...
        return socket.emit(
          'error',
          'The event sent is not recognized by the server.'
        );
      }
      ...
      if (handler) {
        ...
        // if a handler isn't found
        console.warn(
          `No handler for event ${eventName} in state ${currentQuiz.state}`
        );
        socket.emit(
          'error',
          `Event ${eventName} not allowed in state ${currentQuiz.state}`
        );
      }
    });
    ...
    socket.on('disconnect', (reason) => {
      const discQuizState = Array.from(activeQuizStates.values()).find(
        (session) => session.socketIdToUserId.hasOwnProperty(socket.id)
      );

      if (!discQuizState) {
        console.warn(
          `Unknown WebSocket ${socket.id} tried to disconnect from session.`
        );
