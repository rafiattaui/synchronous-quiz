...
export default function AdminView({ state, pongServer, status, quizId, players, isAdmin=false}: IAdminView) {
    ...
    return (
        <div className="relative z-50 p-6 max-w-2xl mx-auto space-y-6 bg-white shadow-lg rounded-xl border mt-10">
            ...
            {isAdmin && (
                <p className='text-sm text-gray-500'>You are the admin of this quiz. Use the controls below to manage the quiz flow.</p>
            )}
            ...
            <div className="mt-6">
                ...
                <pre className="bg-slate-900 text-emerald-400 p-4 rounded-lg mt-2 overflow-x-auto text-xs leading-relaxed">
                    {players && Object.keys(players).length > 0 ? JSON.stringify(players, null, 2) : "// No players in state"}
                </pre>
            </div>
        </div>
    )
